...
MODEL_PATH = os.path.join(ROOT_DIR, "weights", "resnet18_flip_invariant.pth")
...
# ============================================================
# FLIP-INVARIANT MODEL
# ============================================================

class FlipInvariantResNet18(nn.Module):
    """
    ResNet18 with built-in flip invariance.

    How it works:
    - Takes an input image
    - Passes it through the network normally
    - Also passes a horizontally flipped version through
    - Averages the two predictions

    This makes the model robust to left/right orientation without
    needing flip augmentation during training.
    """
    ...
    def forward(self, x):
        """Forward pass with flip averaging."""
        # Get prediction for original image
        logits_original = self.backbone(x)

        # Flip image horizontally and get prediction
        x_flipped = torch.flip(x, dims=[3])
        logits_flipped = self.backbone(x_flipped)

        # Average the two predictions
        return (logits_original + logits_flipped) / 2.0

    def forward_with_branches(self, x):
        """
        Forward pass returning original, flipped, and averaged predictions.
        Useful for analysis and debugging.
        """
        logits_original = self.backbone(x)
        logits_flipped = self.backbone(torch.flip(x, dims=[3]))
        logits_avg = (logits_original + logits_flipped) / 2.0
        return logits_original, logits_flipped, logits_avg
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("FLIP-INVARIANT RESNET18 TRAINING")
    print("="*70)
    print(f"Seed: {SEED}")
    ...
    # Note: No horizontal flip - the model handles that internally!
    ...
    print("\nNote: RandomHorizontalFlip removed - model handles flips internally")
    ...
    model = FlipInvariantResNet18().to(device)
